...
Please note that left mouse click deletes vertexes.  For a more real world application `franconia[5,]` from `mapview` has a hole.  Try to edit it with the following code.
...
`Leaflet.pm` gives us a very pleasant snapping experience, so if you want to snap, set `editor = "leafpm"` and snap away.  Here is how it looks with the example from above.

![screenshot of snapping](../images/mapedit-leafpm-2.gif)

Snapping is enabled by default.  Turn snapping off by setting `snappable = FALSE` in `pmCutOptions()`, `pmEditOptions()`, and/or `pmDrawOptions()`.  `snapDistance` with these `pm*Options()` functions adjusts the sensitivity of the snapping.
...
We sort of backed into a solution with `MULTILINESTRING` since `Leaflet.pm`'s approach fits better with `MULTI*` features.  As an example, let's edit one of the `trails` from `mapview`.
...
As of this post we have reached the end of the extremely generous [RConsortium](https://www.r-consortium.org/) funding of `mapedit`.  Although the funding is over, we still expect to actively maintain and improve `mapedit`.  Your participation, ideas, and feedback are as vital as ever.  Thanks to all those who have contributed so far and thanks to all open source contributors in the R and JavaScript communities.

...
way. A more extensive viewer of the data collections is obtained by
```{r eval=FALSE}
collection_viewer()
```
...
Satellite image tiles are associated with the time of overpass of the satellite. For larger areas, these times will differ. Although one can view image collections with different observation times for different regions as sparse datacubes, it is often easier to first aggregate all the data to a common time basis in which it forms a dense datacube. More explanation about datacubes in openEO is found [here](https://openeo.org/documentation/1.0/datacubes.html).

We will now process the NDVI values to a monthly series, by picking for each pixel the median value of all pixels over the month (Sentinel-2 has an image for roughly every 5 days). This is done by `aggregate_temporal_period`:
```{r}

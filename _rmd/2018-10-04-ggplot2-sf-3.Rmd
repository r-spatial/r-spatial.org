...
The package `ggplot` has a faceting option, to plot multiple variables of a dataset, at once, without specifying the location of the figure in the plot area. In order to successfully `facet_wrap`, the `sf` data has to have columns with variables, that can be faceted. Normally, this argument can work with any data frame, but it can also be useful in mapping objects, especially is there is additional information represented per region, such as population estimate in the example below.

```{r ggplot_usa, fig.width = 12, fig.height = 9, out.width = "100%"}

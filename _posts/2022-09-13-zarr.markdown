...
-   we set the CRS of the precipitatoin data cube, equal to that of NE
    (WGS84)
-   we call `st_make_valid()` to make the country boundaries [valid *on
    the
    spere*](https://keen-swartz-3146c4.netlify.app/spherical.html#validity-on-the-sphere)
...
and returs a *vector data cube*: an array with 241 country geometries x
...
interpolation](https://keen-swartz-3146c4.netlify.app/featureattributes.html#area-weighted);
...
We can plot the first 49 time slices e.g. by
    ...
    plot(r[,,,1:49]) # first 100 time steps
    ## downsample set to 2
